...
import { CreateFoundCardRequest } from "@/types/foundCard";

const FoundLostCard = () => {
  const { user, getIdToken } = useAuth();
  const [selectedOption, setSelectedOption] = useState<string>("");
  const [formData, setFormData] = useState<CreateFoundCardRequest>({
    fullName: "",
    phone: "",
    email: "",
    idType: "",
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    if (!user) {
      setSubmitMessage("Please sign in to submit a found card");
      return;
    }
    ...
    try {
      ...
      const response = await fetch("/api/found-cards", {
        method: "POST",
        headers: {
      ...
      if (result.success) {
        setSubmitMessage("Found card submitted successfully!");
        setFormData({
          fullName: "",
          phone: "",
          email: "",
        ...
        const errorMsg = result.details
          ...
          : result.error;
        setSubmitMessage(errorMsg || "Failed to submit found card");
      }
    } catch (error) {
          ...
          <div className="rounded-sm border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark">
            ...
            <form onSubmit={handleSubmit}>
              <div className="p-6.5">
                ...
                >
                  {isSubmitting ? "Submitting..." : "Report Found Card"}
                </button>
