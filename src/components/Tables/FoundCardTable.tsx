...
import { FoundCardSubmission } from "@/types/foundCard";
import { LoaderSmall } from "../common/Loader";

const FoundCardTable = () => {
  const { user, getIdToken } = useAuth();
  const [foundCards, setFoundCards] = useState<FoundCardSubmission[]>([]);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    const fetchFoundCards = async () => {
      if (!user) {
        setLoading(false);
        return;
      }
      ...
      <div className="rounded-sm border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark">
        ...
        <div className="p-6 text-center">
          <p className="text-gray-500">
            Please sign in to view your found cards
          </p>
        </div>
      </div>
    );
  ...
  return (
    <div className="rounded-sm border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark">
      ...
      {loading ? (
        ...
        <div className="p-6 text-center">
          <p className="text-gray-500">
            No found cards submitted yet. Report a found ID!
          </p>
        </div>
      ) : (
        foundCards.map((card) => (
