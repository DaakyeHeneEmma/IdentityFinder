...
BEGIN
    ...
    ELSIF similarity(LOWER(r_card.full_name), LOWER(f_card.full_name)) > 0.6 THEN
        score := score + 0.20; -- 20% for similar name
    END IF;

    IF r_card.id_number = f_card.id_number AND r_card.id_number IS NOT NULL THEN
        score := score + 0.25; -- 25% for matching ID number
    END IF;

    IF r_card.phone_number = f_card.phone_number AND r_card.phone_number IS NOT NULL THEN
...
INSERT INTO public.users (id, email, name, occupation, bio) VALUES
    ('00000000-0000-0000-0000-000000000001', 'demo@identityfinder.com', 'Demo User', 'Software Engineer', 'This is a demo user account for testing purposes.');
